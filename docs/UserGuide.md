...
### Locating clients by name, phone or email: `find`

Finds clients whose names, phones or emails contain any of the given keywords.

Format: `find k/KEYWORD k/[MORE_KEYWORDS]`

* The search is case-insensitive. e.g `hans` will match `Hans`
* The order of the keywords does not matter. e.g. `Hans Bo` will match `Bo Hans`
* Only the name, phone and email are searched.
* Only full words will be matched e.g. `Han` will not match `Hans`
* Persons matching at least one keyword will be returned (i.e. `OR` search).
  e.g. `Hans Bo` will return `Hans Gruber`, `Bo Yang`

Examples:
* `find k/John` returns `john` and `John Doe`
* `find k/alex k/david` returns `Alex Yeoh`, `David Li`<br>
